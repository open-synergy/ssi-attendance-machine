# Copyright 2026 OpenSynergy Indonesia
# Copyright 2026 PT. Simetri Sinergi Indonesia
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).

from odoo.tests import HttpSavepointCase, tagged


@tagged("post_install", "-at_install")
class TestUiAttendanceMachineModel(HttpSavepointCase):
    """Tour tests for the ``attendance_machine_model`` work instructions.

    Uses ``HttpSavepointCase`` rather than plain ``HttpCase``: in 14.0
    ``TransactionCase`` only assigns ``self.env`` inside instance
    ``setUp()``, so ``cls.env`` is not available in ``setUpClass()``.
    ``HttpSavepointCase`` (via ``SingleTransactionCase``) does set
    ``cls.env`` in ``setUpClass()``, which the tours below rely on to
    seed records visible to the browser session.
    """

    @classmethod
    def setUpClass(cls):
        """Grant the configurator group and seed the records the tours use."""
        super().setUpClass()
        # Pre-Condition: the Attendance Machine Models menu is gated by the
        # "Attendance Machine Model" group. Without it the tour dies on its
        # first step -- the menu is never rendered for "admin".
        cls.env.ref(
            "ssi_attendance_machine.attendance_machine_model_group"
        ).sudo().write({"users": [(4, cls.env.ref("base.user_admin").id)]})
        cls.model_edit = cls.env["attendance_machine_model"].create(
            {
                "name": "TOUR-MODEL-EDIT",
                "code": "/",
            }
        )
        cls.model_delete = cls.env["attendance_machine_model"].create(
            {
                "name": "TOUR-MODEL-DELETE",
                "code": "/",
            }
        )
        cls.model_deactivate = cls.env["attendance_machine_model"].create(
            {
                "name": "TOUR-MODEL-DEACTIVATE",
                "code": "/",
            }
        )
        cls.model_activate = cls.env["attendance_machine_model"].create(
            {
                "name": "TOUR-MODEL-ACTIVATE",
                "code": "/",
                "active": False,
            }
        )
        cls.model_reset_code = cls.env["attendance_machine_model"].create(
            {
                "name": "TOUR-MODEL-RESET-CODE",
                "code": "TOUR-RESET-CODE-001",
            }
        )

    def test_create(self):
        """Run the create tour for ``attendance_machine_model``.

        IK: docs/attendance_machine_model/01-create.md
        """
        self.start_tour(
            "/web",
            "ssi_attendance_machine_attendance_machine_model_create",
            login="admin",
        )

    def test_edit(self):
        """Run the edit tour for ``attendance_machine_model``.

        IK: docs/attendance_machine_model/02-edit.md
        """
        self.start_tour(
            "/web",
            "ssi_attendance_machine_attendance_machine_model_edit",
            login="admin",
        )

    def test_delete(self):
        """Run the delete tour for ``attendance_machine_model``.

        IK: docs/attendance_machine_model/03-delete.md
        """
        self.start_tour(
            "/web",
            "ssi_attendance_machine_attendance_machine_model_delete",
            login="admin",
        )

    def test_deactivate(self):
        """Run the deactivate tour for ``attendance_machine_model``.

        IK: docs/attendance_machine_model/04-deactivate.md
        """
        self.start_tour(
            "/web",
            "ssi_attendance_machine_attendance_machine_model_deactivate",
            login="admin",
        )

    def test_activate(self):
        """Run the activate tour for ``attendance_machine_model``.

        IK: docs/attendance_machine_model/05-activate.md
        """
        self.start_tour(
            "/web",
            "ssi_attendance_machine_attendance_machine_model_activate",
            login="admin",
        )

    def test_reset_code(self):
        """Run the reset code tour for ``attendance_machine_model``.

        IK: docs/attendance_machine_model/06-reset-code.md
        """
        self.start_tour(
            "/web",
            "ssi_attendance_machine_attendance_machine_model_reset_code",
            login="admin",
        )
