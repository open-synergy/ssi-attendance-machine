...
> **Module:** ssi_attendance_machine_documenso_signing
>
...
- **Module:** `ssi_attendance_machine_documenso_signing` is installed.
- **Config:** The record's active Approval Template has a Documenso Signing Template
  configured. When it does not, this delta does not apply and the base Flow
  (`ssi_attendance_machine/docs/attendance_machine_import/05-approve.md`) is unchanged.
...
- The behavior below only applies **when the record's active Approval Template has a
  ...
  `ssi_attendance_machine/docs/attendance_machine_import/05-approve.md`.
...
- Instead of clicking Approve, the user opens the linked request — shown under the
  ...
  **Signature Requests** tab (or via the **Open Signature Requests** button in the same
  ...
  Odoo, in Documenso.
- Approval completes automatically — without any button click on this Attendance Machine
  Import record — once the linked signature request reaches the **Signed** status. At
  that point, the base Post-Condition applies as usual: if this was the last pending
  approval level, status automatically changes to **Queue Done** via
  `action_queue_done`; otherwise status remains **Waiting for Approval** and the next
  level becomes pending.
- If the linked signature request is **cancelled** instead (e.g. a signer declines in
  ...
  Reject action (`06-reject.md`), but triggered automatically by the cancelled request
  rather than by a user clicking Reject.
